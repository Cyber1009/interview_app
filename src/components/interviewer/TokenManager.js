...
const TokenManager = ({
  ...
  const getTokenStatus = (token) => {
    // With the updated API schema, simply use is_used to determine status
          ...
          <Card elevation={0} sx={{ 
            ...
            <CardContent sx={{ p: 2.5 }}>
              {loading ? (
                ...
                <Grid container spacing={2}>                  {tokens.map((token, index) => {
                    // Get token value using the updated backend API schema (token_value is the field name)
                    const tokenValue = token.token_value;
                    
                    // Skip rendering if token doesn't have a value
                    if (!tokenValue) return null;
                    
                    // With the updated API schema, we primarily use is_used for status determination
                    const status = token.is_used ? 'used' : 'active';
                    const expiryDate = token.created_at ? new Date(token.created_at) : null;
                    ...
                    return (
                      <Grid item xs={12} sm={6} md={4} key={token.id || index}>                        <Paper 
                          elevation={0} 
                          sx={{ 
                            p: 3,
                            border: '1px solid',
                        ...
                        >
                          ...
                          <Box sx={{ pt: 1 }}>                            <Typography variant="h6" sx={{ 
                              fontSize: '1rem',
                              ...
                              {status === 'used' ? 'Used' : 'Active'} • Created: {new Date(token.created_at).toLocaleDateString()}
                            </Typography>
                            <Box sx={{ 
                              mt: 2,
                              pt: 2,
